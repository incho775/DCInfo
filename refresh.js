...
function checkReply() {
    if (SETTING.ischeckip || SETTING.isnickid) {
...
function setPagination() {
    ...
    pagelist.attr('id', 'dcinfo_pagination');
        ...
        getData(event.target.href).then(data => {
            ...
            if(event.target.className === "" ){ //페이지 숫자 클릭시
                ...
                for (let elem of newpagelist_elem){
                    if(elem.id != "dcinfo_pagination" && elem.parentNode.parentNode.localName == 'article'){
...
function setAutoRefresh() {
    ...
    let mainloop = null;
    const interval = SETTING.refreshtime * 1000;
    setLoader(0, 'init');
    setLoader(SETTING.refreshtime, "update");
    ...
    mainloop = setTimeout(function ajax() {
        ...
        getData(window.location.href).then(document => {
            document && setLoader(SETTING.refreshtime, "update");
    ...
    document.addEventListener("visibilitychange", function(){
        if (document.hidden) {
            ...
            if (mainloop === 0 && SETTING.isrefresh) {
                setLoader(0, "init");
                setLoader(SETTING.refreshtime, "update");
